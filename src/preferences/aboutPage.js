...
class AboutPage extends Adw.PreferencesPage {
    ...
    constructor(metadata, settings, wnd) {
        ...
        let openMeteoImage = new Gtk.Image({
            icon_name: "open-meteo",
            margin_bottom: 5,
            pixel_size: 150,
        });
        let openMeteoLabel = new Gtk.Label({
            label: '<span size="larger"><b>Open-Meteo GNOME</b></span>',
            use_markup: true,
            margin_bottom: 15,
            vexpand: true,
        ...
        let aboutDescription = new Gtk.Label({
            label: _(
                "Weather forecast powered by Open-Meteo for any location on Earth in GNOME Shell"
            ),
            margin_bottom: 3,
            hexpand: false,
            vexpand: false,
        ...
        aboutBox.append(openMeteoImage);
        aboutBox.append(openMeteoLabel);
        ...
        // Extension version
        let openMeteoVersionRow = new Adw.ActionRow({
            title: _("Open-Meteo GNOME Version"),
        });
        openMeteoVersionRow.add_suffix(
            new Gtk.Label({
        ...
        infoGroup.add(openMeteoVersionRow);
        ...
        let repoLink1 = "<a href=\"https://github.com/wwktz\">%s</a>";
        let repoLink2 = "<a href=\"https://github.com/wwktz/gnome-openmeteo\">%s</a>";
        let maintainerAbout = new Gtk.Label({
            label: _("Maintained by: %s").format(repoLink1.format("Weikang Wang")),
            use_markup: true,
            hexpand: false,
            vexpand: false,
        });
        let contribute = new Gtk.Label({
            label: _("Contribute or translate the project on %s.").format(repoLink2.format("GitHub")),
            use_markup: true,
            hexpand: false,
            vexpand: false
        });
        let fork = new Gtk.Label({
            label: _("This is a fork of %s's extension.").format("Teal Penguin"),
            use_markup: false,
            hexpand: false,
            vexpand: false
        });
